...
const ReportsTable: React.FC<ReportsTableProps> = ({ data }) => {
  ...
  const columns = [
    ...
    { id: 'context', label: 'شناسه', icon: <TbLockCog size={18} />, isCopyable: true },
    ...
    { id: 'status', label: 'اعلام وضعیت', icon: <GrStatusUnknown size={18} /> },
    { id: 'icon' },
  ]
  const rows = data.map((report, index) => ({
    ...
    status:
      report.status === 'SUCCESS' ? (
        <div style={{ color: theme.palette.success.main }}>موفق</div>
      ) : (
        <div style={{ color: theme.palette.error.dark }}>ناموفق</div>
      ),
    icon: (
      <LuEye
        size={24}
